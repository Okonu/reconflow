...
| 30 | Zero-configuration Docker (owner: "it should run without configuration") | `docker-compose.yml` is at the repo root. Every setting has a default, and APP_KEY, the PII salt and the source token are generated into the `app_secrets` volume on first boot. The demo password is `ReconFlow2026` and the Postgres password `reconflow` (internal network only); both can be overridden in `deploy/.env`. It serves plain HTTP on port 8080; HTTPS turns on when `SITE_ADDRESS` is a domain |
| 31 | AI without a key in Docker | When `ANTHROPIC_API_KEY` is empty and `AI_DRIVER` is unset, the stub driver runs, so AI screens work in the interview. Each suggestion says "Offline rules stub (no AI key configured)". Setting a key switches to Claude |
| 32 | First-boot data | Seeding runs in the worker after the app is healthy. The dashboard shows "Preparing demo data…" and polls until the first run completes |

## Test phase (after all phases, owner instruction)

- **Result:** 373 tests and 1,635 assertions pass. The slow group reconciles 50,000 sales in 7.8 s (budget 60 s; an earlier 16.1 s reading was taken while Docker builds were running on the same machine). Pint, Larastan (level 5), ESLint and tsc are clean. Report: `docs/test-report.md`, generated by `php scripts/test-report.php` (CI regenerates it with coverage).
- **Found and fixed:**
  - `ReportRequest::format()` clashed with `Request::format()`. This was a fatal error on the report pages that made Pest exit silently.
  - The AI batch job lazy-loaded results (N+1).
  - The permission contract was extended to all new routes. To keep one permission per route, AI triage/summary no longer also require viewing the exception or run, the settings page needs `config.view`, and audit export authorises on `audit.export` alone.
  - `phpstan.neon` now includes root migrations.
  - `APP_NAME` defaults to ReconFlow.
- **Docker verification:**
  - `make down && make up` builds and starts all five services, and the app is healthy.
  - `/health` and `/ready` are OK, and `/metrics` returns 404 at the edge.
  - Every page returns 200 for each demo role, or 403 where that role lacks the permission.
  - The XLSX export downloads.
  - The admin demo reset regenerates 14 days through the worker.
- **New tests:**
  - AI governance: redaction, input and prompt hashes, override reason, no workflow writes, schema rejection, kill switch, no-key message, aggregate-only narrative, batch triage, eval set, oversight permissions.
  - Notifications: run blocked, critical aggregation, daily summary with a Slack payload free of PII, own-inbox isolation.
  - Dashboard.
  - Settings: sections, versioning, band validation, SLA and threshold effects, AI toggle.
  - Report: filters, masked CSV, formula escaping, XLSX, unmasked export needing permission and reason.
  - Unmask, erasure and retention.
  - Audit export and filters.
...
- **Security pass:**
  - no secrets tracked (`.env` and `deploy/.env` are git-ignored; only examples are committed; Docker generates secrets on first boot);
  - restrictive CORS;
  - login throttling;
  - Argon2id hashing;
