...
Each entry also has an **AI-assisted tooling** field. It gives my direction or decision first. Then it gives what the AI tools made, and what I changed or rejected.
...
- **Status:** Accepted
- **AI-assisted tooling:** _My direction and decision:_ I accepted the recommendation. I kept "buy" as a check in the operating model, not as a rejected option. _What the AI tools made:_ Claude compared six approaches (spreadsheets, RPA, low-code, buy, AI matching, code-first) and recommended code-first.
...
- **Status:** Accepted
- **AI-assisted tooling:** _My direction and decision:_ I made the answer keys the authority for rule behaviour. When the prompt and the answer keys did not agree on R6, I selected the answer keys. _What the AI tools made:_ Claude proposed the rule set R1 to R7. Claude also made the golden and volume datasets and the answer keys, and checked them with an independent reference matcher.
...
- **Status:** Accepted
- **AI-assisted tooling:** _My direction and decision:_ I questioned the AI principle in the prompt, and Claude removed the AI layer. After Claude explained its purpose, I decided to keep it, as an assistant only. _What the AI tools made:_ Claude put AI exception triage in the build prompt.
...
- **AI-assisted tooling:** _My direction and decision:_ I made AI governance and data protection a requirement for the full design, not only for the AI feature. _What the AI tools made:_ Claude wrote the AI governance and data protection section of the build prompt. Claude Code wrote the `Redactor` and the tests.
...
- **Status:** Accepted
- **AI-assisted tooling:** _My direction and decision:_ I rejected this advice and selected Laravel. Claude then converted the build prompt and the rules file to Laravel. _What the AI tools made:_ Claude recommended Python and FastAPI. When I asked about Laravel, Claude advised me to keep FastAPI.
...
- **AI-assisted tooling:** _My direction and decision:_ I selected my own server. I required a stack that starts with no configuration, for the panel. Claude Code changed the setup to meet this requirement. It deployed the system, to my direction, in the isolated demo environment of D-19. _What the AI tools made:_ Claude proposed Docker Compose and a managed host. Claude Code built the Compose stack and the CI pipeline.
...
- **Status:** Accepted
- **AI-assisted tooling:** _My direction and decision:_ My engineering rules require that the server, not the browser, enforces all access and approval controls. _What the AI tools made:_ Claude specified maker-checker and the $1,000 threshold. Claude Code wrote the policy and the tests.
...
- **Status:** Accepted
- **AI-assisted tooling:** _My direction and decision:_ I set the retention of the audit log (7 years, then an archive with a checkpoint). _What the AI tools made:_ Claude specified the hash chain. Claude Code wrote the audit module, the trigger and the verifier.
...
- **Status:** Accepted
- **AI-assisted tooling:** _My direction and decision:_ I decided how a new run keeps exceptions, comments and adjustments. _What the AI tools made:_ Claude specified run versions and idempotency keys. Claude Code wrote them.
...
- **Status:** Accepted
- **AI-assisted tooling:** _My direction and decision:_ I decided that amounts with more than 2 decimal places go into quarantine and are never rounded. I also decided the rules for duplicate IDs. _What the AI tools made:_ Claude specified the data checks. Claude Code wrote them and proposed more checks.
...
- **AI-assisted tooling:** _My direction and decision:_ I accepted the design. _What the AI tools made:_ Claude specified the connector interface and the simulated source APIs. Claude Code wrote them.
...
- **Status:** Accepted
- **AI-assisted tooling:** _My direction and decision:_ I asked for seed data, templates that users can download, and a preview of each upload. These were not in the first prompt. _What the AI tools made:_ Claude designed the upload flow and made the Excel templates. Claude Code wrote the staging, the preview and the template builder.
...
- **Status:** Accepted
- **AI-assisted tooling:** _My direction and decision:_ I decided the escalation: one day of carry-forward, then `MISSING_PAYMENT`. _What the AI tools made:_ Claude proposed the timing rule and the carry-forward.
...
- **AI-assisted tooling:** _My direction and decision:_ I accepted the values. Finance must confirm them. _What the AI tools made:_ Claude proposed the default values.

### D-15 · Edge-case rules (my decisions on 2026-09-23 and 2026-09-24)
- **Decision:** I decided these rules. [Reconciliation rules](reconciliation-rules.md) and [Assumptions](assumptions.md) give the full text.
...
- **AI-assisted tooling:** _My direction and decision:_ I decided each answer and sent it to the coding agent. When time was short, I told the agent to record new edge cases with a default, and not to stop. _What the AI tools made:_ Claude Code raised each edge case as a question. Claude proposed an answer for each question.
...
- **AI-assisted tooling:** _My direction and decision:_ I accepted them, including the credit-sales limit. _What the AI tools made:_ Claude proposed the scope limits.
...
- **AI-assisted tooling:** _My direction and decision:_ I accepted the order. _What the AI tools made:_ Claude proposed the priority order for the last day.
