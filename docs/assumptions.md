...
- **Carried PENDING_TIMING sales** are matched against D+1 payments with R1, R2 and R3. Fuzzy matching uses the same constraints (same phone, amount within tolerance, \|Δt\| ≤ 24 h from the sale timestamp, one-to-one, ties not matched) in a single pass with the current day's sales, so a payment that fits both is a tie. The result is MATCHED_FUZZY in the prior-day section, flagged as needing confirmation. *(Owner, 2026-09-24.)*
- **Escalated and missing-payment items in the lookback** are auto-matched on exact reference (or R2 split) only. Unmatched later payments from the same phone with an amount within tolerance, within the lookback, are offered as possible matches. An Analyst (`matches.confirm`) can confirm one with a reason. That creates an audited manual match (who, when, reason) and resolves the item as "Paid late (D+n), manually matched". The match is stored separately and re-applied on any re-run, so it survives re-running either date. *(Owner, 2026-09-24.)*
