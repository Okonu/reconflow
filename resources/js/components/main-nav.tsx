...
const LINKS = [
    { label: 'Dashboard', route: 'home', permission: 'dashboard.view' },
    { label: 'Runs', route: 'runs.index', permission: 'runs.view' },
    { label: 'Report', route: 'reports.reconciliation', permission: 'results.view' },
    ...
    { label: 'Settings', route: 'settings.index', permission: 'config.view' },
];
            ...
            {LINKS.filter((link) => can(link.permission)).map((link) => (
                <Link
                    ...
                    className={cn(
                        ...
                        (link.route === 'home' ? route().current('home') : route().current(`${link.route.split('.').slice(0, -1).join('.')}.*`)) && 'bg-accent text-accent-foreground',
                    )}
                >
                    {link.label}
                </Link>
