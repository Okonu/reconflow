...
final class AdjustmentPolicy
{
    ...
    public function approve(User $user, Adjustment $adjustment): Response
    {
        ...
        $threshold = $this->approvals->threshold();
        if (($adjustment->high_value || $adjustment->amount?->isGreaterThan($threshold)) && ! $user->can(AdjustmentPermission::ApproveHighValue->value)) {
